...
### Phase 6 — Reference image to public (Lane 2) `[x]`
...
### 2026-08-01 — D7 built: source mode retired outright, and the stale recipe copy turns out to have been holding up vacuous tests

Phase 7's engine half is built. Two decisions and one discovery, all of which change what the remaining phases have to do.

**Decision 1 — `resolve_assets` collapses to ONE mode; the source-checkout branch is retired, not preserved (Zach, 2026-08-01).** The draft left an opening to keep source mode alive by re-aiming it at a public sibling checkout, so `team-images.yml` could keep catching "a framework change on main breaks a team's bake" before release. Rejected on principle: **`moda-agents` consumes RELEASED `bobi-agent` versions, and `bobi-agent` owns the machinery that keeps its releases from breaking consumers.** A fleet repo gating the framework's tip-of-main is the dependency running backwards. So `find_repo_root`, `_checkout_build_args`, `_resolve_build_mode`, and the `--build source|pypi|wheel` CLI flag are all gone; `BOBI_BUILD` is gone from `build-team-images.sh`. What remains is `overlay`: the published base plus the team's rendered hook. `bobi build` can no longer bake unreleased framework code at all, deliberately.

**Decision 2 — the overlay is a committed file plus a guard test, not generated text (Zach, 2026-08-01).** `bobi-deploy/docker/team.Dockerfile`, force-included into the wheel. Because a Dockerfile cannot read `BUILD_SECRET_NAMES`, the drift seam that argued for generating it is closed by a test instead: `test_team_overlay_mounts_every_build_secret` asserts the file's `--mount=type=secret,id=` set equals that tuple, that its `FROM` names `BASE_IMAGE_REPO`, and that its `ARG TEAM_DEPS` default is the path `resolve_assets` actually stages. Adding a build secret without adding its mount now fails CI rather than building without the secret.

**Discovery — the stale recipe copy was not inert; it was propping up tests that proved nothing.** Phase 6 published the recipe but did not delete `bobi-deploy`'s copy (correctly — that tree is frozen, and Phase 7 deletes in the merged copy). What was not noticed is that a large body of tests kept reading that copy and kept passing against it. Deleted here with their subject:

- **36 `test_entrypoint_*` tests** in `tests/test_gitops_c22.py` (592 + ~590 lines across two removal passes), asserting gateway auth, codex credential materialization, and supervision. They were reading a **pre-0.51.0 entrypoint** — two of them still demanded the `bobi-supervisor` shim and `BOBI_SUPERVISOR_SRC` ARG that Lane 1 replaced with `bobi agent <name> supervise`. They asserted the old shape, passed, and told us nothing about what ships.
- **`tests/test_container_contract.py`** and five Dockerfile tests in `bobi_deploy/tests/test_packaging.py` (build stages, the aichat pin, the fastembed-not-`[kb]` guard, the supervisor bundling) — same stale subject.
- **`bobi-deploy/tests/integration/test_container_image.py`**, byte-identical to `bobi-agent`'s copy. Pure duplicate; nothing lost.

This is the same class as the `pypi.yml` false-match that cost real time earlier in this lane, and as Lane 1's unaccounted-for supervisor tests: **a duplicated file keeps its tests green while the real file moves out from under them.** The general lesson for the rest of the reorg: when a file is published, grep for what *reads* it, not just what *imports* it.

**Consequence — orphaned coverage that must be re-homed in `bobi-agent`, tracked separately.** The public repo has *no* image-level coverage of the `TEAM_DEPS` hook it publishes as a consumer contract (D6): the only trace is a path-filter entry for `bobi/build_render.py` in `container.yml`. `container.yml`'s own comment records the `bobi build` integration and dependency-bootstrap e2e as staying private, which is now wrong — those exercise a public recipe. Also missing publicly: a `builder-wheel` assertion, and the entrypoint tests above.

**Also found, NOT actioned (needs its own decision).** `bobi_deploy/src/bobi_deploy/supervisor/` is still a second copy of public `bobi/supervisor/` — 8 of 11 modules byte-identical, 3 diverged where Lane 1 evolved the public one. Nothing in production imports it; only its own tests do. It carries **361 lines of `test_supervisor_alerting.py` that have no public counterpart at all**. Deleting it is not forced by anything in this phase, and whether that alerting coverage is redundant or genuinely unique needs a diff rather than an assumption — so it stays for now, flagged.

**Smaller decisions folded in rather than parked.** `claude_version:` in a deployment config is now refused with an explanation: a team overlay cannot re-pin a CLI the base already baked, and left plumbed it would become a build-arg the overlay has no `ARG` for — which docker reduces to a warning, so the key would read as honoured while changing nothing. `provision-instance.sh`'s `--build-context`/`--dockerfile` defaults (`$REPO_ROOT` + `$REPO_ROOT/Dockerfile`) are now required flags, since after the deletion those defaults could only ever resolve to a missing file.

**Verified, not assumed.** The overlay was built for real against the published `ghcr.io/moda-labs/bobi:0.51.1`: `roadmap-pm`'s hook ran (apt `jq`, the `gh` keyring/install), its own `verify requires` gate passed inside the build, and the resulting image carries the `#379` deps stamp (`/opt/bobi/team-deps.hash`), `gh 2.97.0`, `jq-1.7`, `bobi 0.51.1`, a working `bobi agent … supervise`, and the inherited entrypoint. The deploy wheel builds and its `_deploy` payload is exactly three scripts plus the overlay — no recipe. 499 engine tests pass; `shellcheck` and `actionlint` clean. Local build wall-clock is not the D7 number to check: the hook layer ran in 7.4s and a cached rebuild in 2s, but the ~4-minute estimate is about the Fly-side roll, which is still unmeasured.
...
### 2026-07-31 — Lane 2 landed; gate (b) closed by overwriting `0.51.1`, and two rules for Lane 3

Both halves merged — `bobi-agent#898` as `37f07b3`, `bobi-deploy#49` as `d0f22ac` (admin merge, authorized under the throwaway-repo directive) — with post-merge CI green on both: 12/12 checks on `37f07b3`, 6/6 on `d0f22ac`.

**Gate (b) — decided and run (2026-07-31, Zach): re-publish `0.51.1` from `bobi-agent`.** There is no non-destructive way to run this proof. `release-image.yml` has no dry-run mode (`push: true` is unconditional) and no tag-suffix input, so *any* dispatch overwrites an already-published version tag with different bytes; and dispatching `0.51.1` also moves `:latest`, since the workflow moves it whenever `v<version>` equals `releases/latest`. The blast radius was verified nil rather than assumed: moda's fleet does not consume `ghcr.io/moda-labs/bobi` (it bakes its own to `registry.fly.io`), and there are no external consumers yet. The decisive argument is that the overwrite is *corrective*, not collateral — the tag being replaced held the bobi-deploy-built, shim-carrying image, i.e. the exact artifact this reorg exists to retire, so the publish points `:latest` at the shim-free image a new consumer should get. Dispatched with `version=0.51.1`, `claude-version=2.1.212`, `source-sha=37f07b3`. The outgoing digest `sha256:b1660b4421ab916eeb45d090013d41778db4777cf2b97c696defee76171b7490` survives untagged in GHCR's version list. The alternative the handoff floated — publish a throwaway prerelease — is not available: pypi mode installs `bobi==<version>` from PyPI, so it would require cutting a real PyPI prerelease purely to satisfy this gate.

Confirmed on the dispatch: the `release` environment was auto-created with **zero protection rules**, as predicted above. Add required reviewers before Lane 3 makes `bobi-agent` the production image-publish path.

**Rule for Lane 3, part 1 — `bobi-deploy` changes only when an extraction breaks it.** Both changes made there so far were forced, not elective: `#47` deleted a workflow testing a Worker that had moved out, and `#49` deleted force-includes pointing at `../Dockerfile` and `../docker/*.sh` that no longer existed, without which the deploy wheel stops building. Everything else waits for Lane 3. This is what makes the Worker freeze recorded below load-bearing rather than merely cautious: if a production Worker bug lands inside the freeze, the response is to accelerate Lane 3, not to patch a repo scheduled for archive.

**Gate (b) closed — Phase 6 is `[x]`.** [Run 30672941328](https://github.com/moda-labs/bobi-agent/actions/runs/30672941328) published both arch images and the manifest from `bobi-agent`, green. `0.51.1` and `latest` now resolve to `sha256:deac65dd8998a0415d455f7d81fdffcea89cb8c7e7dfc4cf2eff1cf4385623e0`, a two-platform manifest (`linux/amd64` + `linux/arm64`); the previous digest is untagged, as intended. Verified against that exact published digest, not a local rebuild: no `bobi-supervisor` on `PATH` and no file matching `*bobi-supervisor*` anywhere on the filesystem; `bobi --version` reports `0.51.1`; `bobi agent --help` carries `supervise`; all **14 container contract tests pass**; and `docker run --init` produces the gate's tree — `/sbin/docker-init`(pid 1) → `bobi agent smoke supervise -- --foreground`(7) → `bobi.cli agent smoke start --foreground`(45) → `claude`(65) — shutting down on `SIGTERM` through `service.py`'s handler with exit 0 in 0.38s. This closes the published-tag half of gate (c), which the entry above could only prove against a local build. Cosmetic, not chased: the graceful shutdown prints a `SystemExit: 0` traceback as it unwinds a thread join in `bobi/service.py`; pre-existing, unrelated to this move.

**Rule for Lane 3, part 2 — Phase 7 deletes in the merged `moda-agents` copy, never in `bobi-deploy`'s tree.** The narrowing paragraph above says the surviving three `../scripts/*.sh` force-includes, `resolve_assets`' source-checkout branch, and `bobi-deploy`'s own `Dockerfile` are "retired together in Phase 7." Read literally that implies a final round of edits to `bobi-deploy`. It does not: the subtree merge carries all of it into `moda-agents`, and the deletion happens *there*, after `deploy-agent-teams.yml` cuts over to the `FROM ghcr.io/moda-labs/bobi:<version>` overlay. `bobi-deploy` is archived as-is. The same holds for its copy of `release-image.yml` and its GHCR access — Phase 7 revokes the grant and drops the workflow on the `moda-agents` side; it never commits to `bobi-deploy` again. Net: after Lane 2, this plan schedules **zero** further code changes to `bobi-deploy`; the only actions left against that repo are the archive flip and the grant revocation.
...
**Status of the Phase 1-5 markers below: all five are DONE.** They are not flipped in place, and cannot be — see the marker-vocabulary note at the end of this entry.

Delivered as one PR against `bobi-agent` plus a two-file deletion PR in `bobi-deploy` (moda-labs/bobi-deploy#47). Five corrections to the approved text, each verified against the real trees rather than reasoned about. Per the freeze rule they are recorded here, not edited above.

1. **The import guard went GREEN, not red.** The plan's headline proof-of-work claim was that `tests/test_import_boundaries.py` "MUST go red the moment Lane A or B lands" and must be rewritten in the same PR. It did not go red. Every scan is scoped to `src/` and `core/`, so `event-server/worker/` was invisible to all of them — green *because unguarded*, which is strictly worse than red, since the new package had no boundary enforcement at all. Re-aimed to model three sibling packages with one-way arrows (neither leaf may import the other; both classified default-deny; the Worker scanned as a core consumer). Proven non-vacuous with five planted mutants, each failing exactly one assertion.

2. **The supervisor's tests were unaccounted for.** The plan says the 11 modules "move as-is" and never mentions tests. Four files totalling 1,333 lines, plus two process fixtures, moved with them. Leaving them behind would have shipped 2,201 untested lines publicly *and* left `bobi-deploy`'s suite importing a package no longer there.

3. **Gate R's payload citation is wrong, and the wheel and sdist differ.** The Gate R paragraph cites `pyproject.toml:70-73` as the wheel's event-server force-include list; that force-include is `skills`. The real declaration is `_STATIC_INPUTS` + `_SOURCE_GLOBS` at **`bobi/events/artifact.py:18-25`**, reached via `hatch_build.py`'s `source_input_paths`, whose globs are `src/**/*.ts` and `core/src/**/*.ts` — so the **wheel** excludes `worker/` by construction, no edit needed. But `pyproject.toml:87` includes `event-server` as a whole directory, so the **sdist ships the Worker by default**. "Default to NOT adding them" was only ever true of the wheel. The narrower decision left for Gate R: leave the asymmetry (wheel lean, sdist complete — the normal sdist-is-the-source-tree convention), or exclude `event-server/worker` from the sdist too. Recommend leaving it. Verified the wheel build path is unaffected either way — the staged build tree installs 244 packages with no wrangler/miniflare and emits a byte-identical local bundle.

4. **The root vitest config had to be narrowed.** The plan established that the two vitest configs are mutually exclusive but not that the existing one needed changing: its default glob swept `worker/test/*.spec.ts` into the Node run, where `cloudflare:test` does not resolve. It now carries an explicit `include`.

5. **Two CI landmines, both found by running the new job rather than reading it.** The protocol suite's default Worker directory still pointed at `event-server/`, where `wrangler.jsonc` no longer is; and npm workspaces HOIST, so `wrangler` installs to the workspace root's `node_modules/.bin`, not the worker package's — the suite looked only in the package and died with `FileNotFoundError`. Either one would have failed the new required check on its first run.

**Container-token guard restated, not exception-listed.** The supervisor trips the old `docker|dockerfile|flyctl` word-ban six times, once as a runtime value (`identity.py` returns the literal `"docker"` as a platform label). Since #707's premise — "the public product builds no docker images" — is what this plan reverses, the guard now bans *assembling a build context or invoking a container/deploy CLI* and permits *naming a runtime*. Narrower in letter, stronger in effect: a new AST pass catches `subprocess.run(["docker", "build", ...])`, whose split argv the old prose regex never saw. Proven two-sided — four build-context mutants fail, and naming a runtime as a value does not.

**Deliberately left in place.** The private `event-server/` and `supervisor/` sources stay in `bobi-deploy`: its `release-fleet.yml:68` still deploys the production Worker from them, and the deploy wheel still force-includes the supervisor. Both retire in Lanes 2-3. Related gap for Lane 3 to settle: **this plan never says who deploys the production Worker after the move** — Movement 3 migrates `release-fleet.yml` to `moda-agents`, which would leave a private repo deploying a Worker whose sources are public. That works, but it is an unstated seam.

**Lane 1's marker mode is `solo`, not `concurrent`** (decided 2026-07-30, Zach), overriding the Lane map's blanket "every lane is `concurrent`". That rule exists to stop parallel lanes racing marker edits inside feature branches, and Split cut no parallel lanes — Lane 1 is the only thing in flight, with 2 and 3 stacked behind a hand-run release. Its cross-repo half is a two-file deletion touching no markers, and the plan file and main code PR are both in `bobi-agent`. So plan changes ride the PR rather than arriving as status-only commits to main. Lanes 2 and 3 stay `concurrent`.

**Marker-vocabulary finding (affects every plan in this repo, not just this one).** The Phase markers above are heading-level and backticked — `### Phase 1 — … \`[ ]\``. `.github/scripts/check-plan-artifact.sh`'s `normalize_markers` only recognizes **list-item** markers (`^\s*- \[x\]`), so it reads a heading marker flip as an edit to frozen approved prose and fails the PR. The consequence is general: **no plan using heading-style phase markers can ever have those markers flipped after approval**, which is exactly what the Execution contract tells builders to do. That is a genuine conflict between the plan template's marker vocabulary and the freeze check, and it needs fixing in one place or the other — not worked around per plan. Until then, phase completion for this plan is recorded in these amendments.

**RESOLVED — 2026-08-03.** `normalize_markers` now carries a second expression for heading-level markers (`.github/scripts/check-plan-artifact.sh:101`), so `### Phase 1 — … `[ ]`` normalizes to `[@]` exactly like a list item and can be flipped after approval. Verified two ways: the sed normalizes all four marker forms, and PR #924 flipped this plan's own heading-level Convergence gate and passed `Check plan artifacts`. The finding above is kept for the history but is no longer true — do not file work against it. (It nearly generated a duplicate issue during the 0.52.0 close-out, which is the cost of leaving a fixed finding stated in the present tense.)
