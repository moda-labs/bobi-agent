...
- **Seven `TeamRuntime` methods are `LocalRuntime`-only.** `runs`, `overview`,
  ...
  docstring carries the sequencing rule (adding an `@abstractmethod` breaks the
  private consumer's CI the moment it merges), and each says in as many words
...
- **`EventBusRuntime` cannot reach a filesystem.** It is a control-plane client
  (`bobi_deploy/webapp/runtime.py`): every read goes through
  ...
  There is no command that returns runs, overview, or run details, and no
  run-scoped write at all.
- So the private repo **cannot** implement these seven alone. The public
  supervisor has to grow the vocabulary first.
...
### Lane A — public: admin-protocol vocabulary (`bobi-agent`)
...
### Lane B — private: `EventBusRuntime` (`moda-agents`, `bobi-deploy/`)
...
ABC's sequencing rule is not advisory, it breaks the private repo's CI on merge.
