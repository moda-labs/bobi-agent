...
- **2026-07-26** (Lane A merge session, decision by Zach): **D119 resolved downward** - `chat: cli` plus the email surface the pack actually subscribes to, rather than adding a Slack service. Keeps `dogfood-content-review` credential-free as the release-smoke pack, at the cost of that pack never exercising the Slack path in smoke runs. Tests exist for both directions, so reversing this is a marker change, not a rewrite.
