...
from modastack.doctor import _check_bubble_auth
...
def tmp_project(tmp_path):
    ...
    modastack_dir.mkdir()
    state_dir = modastack_dir / "state"
    state_dir.mkdir()
    # Minimal agent.yaml so Config.load works
    (modastack_dir / "agent.yaml").write_text("agent: test\n")
    return tmp_path


def _write_bubble(project: Path, bubble_id: str = "", bubble_key: str = ""):
    state_file = project / ".modastack" / "state" / "bubble.json"
