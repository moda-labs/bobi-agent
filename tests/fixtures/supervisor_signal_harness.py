...
Ported from the public repo's tests/fixtures/watchdog_signal_harness.py. The
acceptance tests drive ``Supervisor.run`` from a worker thread (where
...
Usage: ``python supervisor_signal_harness.py <child-pidfile>``
"""
...
def main() -> int:
    ...
    # Healthy idle forever - the supervisor must never restart; the only way out
