...
# --- Bobi install integrity ---

class FakeRecordHash:
    def __init__(self, mode: str, value: str) -> None:
        self.mode = mode
        self.value = value


class FakeRecordFile:
    def __init__(self, path: str, hash_value=None) -> None:
        self.path = path
...
class FakeDistribution:
    ...
    def locate_file(self, file) -> Path:
        return self.root / str(file)


def _record_sha256(data: bytes) -> FakeRecordHash:
    digest = hashlib.sha256(data).digest()
    value = base64.urlsafe_b64encode(digest).decode().rstrip("=")
    return FakeRecordHash("sha256", value)


class TestCheckBobiInstallIntegrity:
    def test_passes_for_matching_record_hash(self, tmp_path):
        from bobi.doctor import _check_bobi_install_integrity

        installed = tmp_path / "bobi" / "__init__.py"
        installed.parent.mkdir()
        data = b"clean\n"
        installed.write_bytes(data)
        dist = FakeDistribution(
            tmp_path, [FakeRecordFile("bobi/__init__.py", _record_sha256(data))]
        )

        result = _check_bobi_install_integrity(lambda: dist)

        assert result.ok
        assert "clean" in result.detail

    def test_fails_for_mismatched_record_hash(self, tmp_path):
        from bobi.doctor import _check_bobi_install_integrity

        installed = tmp_path / "bobi" / "subagent.py"
        installed.parent.mkdir()
        installed.write_text("patched\n")
        dist = FakeDistribution(
            tmp_path, [FakeRecordFile("bobi/subagent.py", _record_sha256(b"clean\n"))]
        )

        result = _check_bobi_install_integrity(lambda: dist)

        assert not result.ok
        assert "bobi/subagent.py" in result.detail
        assert "Reinstall" in result.hint

    def test_fails_for_missing_hashed_file(self, tmp_path):
        from bobi.doctor import _check_bobi_install_integrity

        dist = FakeDistribution(
            tmp_path, [FakeRecordFile("bobi/missing.py", _record_sha256(b"clean\n"))]
        )
    ...
    def test_passes_when_distribution_metadata_is_unavailable(self):
        from importlib.metadata import PackageNotFoundError

        from bobi.doctor import _check_bobi_install_integrity

        def missing_dist():
            raise PackageNotFoundError("bobi")

        result = _check_bobi_install_integrity(missing_dist)

        assert result.ok
        assert "metadata unavailable" in result.detail

    def test_passes_for_editable_install_without_record_files(self):
        from bobi.doctor import _check_bobi_install_integrity

        dist = FakeDistribution(Path("/tmp/source"), None)

        result = _check_bobi_install_integrity(lambda: dist)

        assert result.ok
        assert "no RECORD hashes" in result.detail

    def test_skips_non_sha256_hashes(self, tmp_path):
        from bobi.doctor import _check_bobi_install_integrity
        ...
        dist = FakeDistribution(
            tmp_path, [FakeRecordFile("bobi/__init__.py",
                                      FakeRecordHash("sha512", "ignored"))]
        )
