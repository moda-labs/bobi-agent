...
def test_track_and_check(tmp_path):
    ...
    store.track("AGD-1", repo_path="/tmp/repo", title="Test",
...
def test_agents_for_repo(tmp_path):
    store = StateStore(path=tmp_path / "state.json")
    store.track("AGD-1", repo_path="/tmp/repo-a", title="A", worktree="/tmp/wt-a")
    store.track("AGD-2", repo_path="/tmp/repo-b", title="B", worktree="/tmp/wt-b")
    store.track("AGD-3", repo_path="/tmp/repo-a", title="C", worktree="/tmp/wt-c")
...
def test_touch_updates_activity(tmp_path):
    store = StateStore(path=tmp_path / "state.json")
    store.track("AGD-1", repo_path="/tmp", title="Test", worktree="/tmp/wt")
...
def test_persistence(tmp_path):
    ...
    store1.track("AGD-1", repo_path="/tmp", title="Test",
...
def test_set_phase(tmp_path):
    store = StateStore(path=tmp_path / "state.json")
    store.track("AGD-1", repo_path="/tmp", title="Test", worktree="/tmp/wt")
    assert store.get("AGD-1").last_phase == ""

    store.set_phase("AGD-1", "triage_complete")
    assert store.get("AGD-1").last_phase == "triage_complete"
