...
modastack history index [--project <filter>]    # index JSONL logs into SQLite
modastack history search <query> [--limit N]    # full-text search across sessions
modastack history sessions [--limit N]          # list indexed conversations
...
The workflow engine (`modastack/workflow/`) executes YAML-based directed acyclic graphs (DAGs) that define multi-step processes. Each workflow is triggered by an event (e.g., `task.assigned`) and runs a series of nodes with dependency tracking and conditional execution.
...
| `action` | Call a registered action (e.g., `session.spawn`, `slack.post`, `ticket.move`) |
| `prompt` | Inject a skill into an engineer session and wait for a handoff phase |
| `manager` | Ask the manager session a question and capture the response |
| `approval` | Wait for a human message (e.g., Slack reply) |
| `gate` | Branch based on conditions, with fallback |

Workflows live in `workflows/` as YAML files. Example: `workflows/issue-lifecycle.yaml` defines the full ticket lifecycle from spawn through PR creation.

```yaml
# workflows/issue-lifecycle.yaml (simplified)
...
nodes:
  ...
  route:
    ...
    branches:
      needs_spec:
        when: "${{triage.needs_spec}} == true"
        goto: spec
      direct:
        goto: implement
```

Key features: topological sort for execution order, `${{scope.variable}}` templating, persistent state per run, async polling for prompt/approval nodes.

### Conversation history

The history indexer (`modastack/history.py`) reads Claude Code JSONL conversation logs from `~/.claude/projects/`, indexes them into SQLite with FTS5 full-text search, and provides a query API.

- **Incremental** — tracks which lines have been indexed per file; only processes new messages
- **Searchable** — full-text search across all past sessions, filterable by project
- **Manager integration** — when events arrive, the manager searches prior conversations for relevant context before making decisions
- **Background indexing** — a daemon thread re-indexes every 120 seconds during `modastack start`

The database lives at `~/.modastack/history.db` with three tables: `conversations` (session metadata, git branch, working directory), `messages` (content, tool calls, timestamps), and `messages_fts` (FTS5 virtual table for search).
...
```
Todo
  │  Issue assigned to bot → manager spawns tmux session + injects /pickup,
...
└── workflow/                     # YAML DAG workflow engine
    ...
    ├── engine.py                 # Core DAG executor — topological sort, async polling
    ├── schema.py                 # Dataclasses (WorkflowDef, NodeDef), YAML parsing
    ├── state.py                  # Persistent workflow run state
    ├── variables.py              # ${{scope.variable}} templating
    ├── actions.py                # Action registry (session.spawn, slack.post, etc.)
    └── triggers.py               # Event-to-workflow dispatcher

workflows/                        # Workflow definitions (YAML DAGs)
└── issue-lifecycle.yaml          # Full ticket lifecycle: spawn → triage → spec → implement → PR
...
| YAML workflows | Declarative DAGs keep orchestration logic out of Python. Manager + gate nodes add LLM reasoning at decision points without hard-coding routing |
| SQLite + FTS5 for history | Local, zero-config, no external service. FTS5 gives fast full-text search. Incremental indexing avoids re-processing |
| Assignment-based pickup | Issues are picked up when assigned to the bot account — explicit, auditable, no ambiguous label semantics |
