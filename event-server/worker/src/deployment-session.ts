...
export class DeploymentSession extends DurableObject<Env> {
	...
	private replayCatchups = new Map<WebSocket, string[]>();
	...
	private async handleIncomingEvent(request: Request): Promise<Response> {
		...
		for (const ws of this.ctx.getWebSockets()) {
			// Replay reads external KV, so another event request can interleave.
			// Hold its live frame until this socket has caught up.
			const catchup = this.replayCatchups.get(ws);
			if (catchup) {
				catchup.push(message);
				continue;
			}
			try {
				ws.send(message);
	...
	private async handleWebSocketUpgrade(request: Request): Promise<Response> {
		...
		this.replayCatchups.set(server, catchup);
		...
		try {
			...
			const url = new URL(request.url);
			const rawLastSeen = url.searchParams.get("last_seen");
			const lastSeen =
				rawLastSeen !== null && /^\d+$/.test(rawLastSeen)
					? Number(rawLastSeen)
					: Number.NaN;
			const replayThrough = this.nextSeq - 1;

			// Zero is a real cursor: the client has processed nothing yet. Invalid
			// and negative cursors remain non-replay requests.
			if (
				Number.isSafeInteger(lastSeen)
				&& lastSeen >= 0
				&& lastSeen < replayThrough
			) {
				await this.replayEvents(server, lastSeen, replayThrough);
			}
			...
			this.replayCatchups.delete(server);
		}
	}
