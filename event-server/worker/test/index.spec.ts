...
describe("DeploymentSession replay cursors", { timeout: 15_000 }, () => {
	...
	async function subscribe(
		deployment: MintedBubble,
		lastSeen: string | null,
	): Promise<WebSocket> {
		const cursorQuery =
			lastSeen === null ? "" : `?last_seen=${encodeURIComponent(lastSeen)}`;
		const response = await SELF.fetch(
			`https://example.com/deployments/${deployment.deployment_id}/subscribe${cursorQuery}`,
			{
				headers: {
	...
	function waitForMessage(
		...
		return new Promise((resolve) => {
			...
			const onMessage = (event: MessageEvent) => {
				const message = JSON.parse(String(event.data)) as SessionMessage;
	...
	function messagesThroughConnected(
		socket: WebSocket,
		timeoutMs: number,
	...
	async function publishEvent(deployment: MintedBubble, topic: string): Promise<void> {
		const body = JSON.stringify({
			source: "test",
			payload: { marker: "cursor-replay" },
		});
		const path = `/events/${topic}`;
		const headers = await bubbleHeaders(
			deployment.bubble_id,
			deployment.bubble_key,
			"POST",
			path,
			body,
		);
	...
	it("replays seq 1 when a process restarts before its first ACK with last_seen=0", async () => {
		const topic = `replay-zero-${crypto.randomUUID()}`;
		const deployment = await mintBubble([topic]);
		const initialSocket = await subscribe(deployment, "0");
		...
		expect(received).toMatchObject([
			{ type: "replay", data: { seq: 1 } },
			{ type: "connected", next_seq: 2 },
		]);
		restartedSocket.close();
	});

	it("delivers events published during replay once, after connected", async () => {
		const topic = `replay-overlap-${crypto.randomUUID()}`;
		const deployment = await mintBubble([topic]);
		await publishEvent(deployment, topic);

		const originalGet = env.EVENTS.get.bind(env.EVENTS);
		let releaseReplayRead = () => {};
		let markReplayReadStarted = () => {};
		const replayReadStarted = new Promise<void>((resolve) => {
			markReplayReadStarted = resolve;
		});
		const replayReadReleased = new Promise<void>((resolve) => {
		...
		try {
			const subscribePromise = subscribe(deployment, "0");
			await replayReadStarted;
			await publishEvent(deployment, topic);
			releaseReplayRead();

			const socket = await subscribePromise;
			...
			expect(await messages).toMatchObject([
				{ type: "replay", data: { seq: 1 } },
				{ type: "connected", next_seq: 3 },
				{ type: "event", data: { seq: 2 } },
			]);
			socket.close();
		} finally {
			releaseReplayRead();
	...
	it.each(["invalid", "-1", "0.5", "-0.5", "0junk", "", null])(
		"does not replay for last_seen=%s",
		async (lastSeen) => {
			const topic = `replay-rejected-${crypto.randomUUID()}`;
			const deployment = await mintBubble([topic]);
			await publishEvent(deployment, topic);
...
describe("#489 internal DeploymentSession auth", () => {
	...
	it("accepts direct DO websocket upgrades with the internal query token", async () => {
		const deploymentId = "direct-good-ws-query";
		const stub = directStub(deploymentId);
		const init = await stub.fetch(
			new Request("https://internal/init", {
				method: "POST",
		...
		const url = new URL(`https://internal/deployments/${deploymentId}/subscribe`);
		url.searchParams.set(INTERNAL_WS_QUERY_PARAM, env.INTERNAL_DO_SECRET);
		const response = await stub.fetch(
	...
	it("websocket subscribe builds a fresh internal request without client auth headers", async () => {
		...
		expect(request.headers.get("Sec-WebSocket-Protocol")).toBeNull();
