...
function makeGitHubBotEvent(overrides: Partial<NormalizedEvent> = {}): NormalizedEvent {
	return {
		...
		delivery: "bulk",
		text: "[moda-labs/bobi-agent] created issue_comment #10",
		fields: { action: "created", number: 10 },
		timestamp: new Date().toISOString(),
		payload: {
			action: "created",
			repository: { full_name: "moda-labs/bobi-agent" },
...
describe("circuit-breaker", () => {
	...
	describe("conversationKey", () => {
		...
		it("extracts github repo#number", () => {
			...
			expect(conversationKey(event)).toBe("github:moda-labs/bobi-agent#10");
		});
		...
		it("keys monitor findings by monitor and stable finding key", () => {
			const event: NormalizedEvent = {
				...
				payload: {
					...
					finding_key: "moda-labs/bobi-agent#519",
				},
			};
			expect(conversationKey(event)).toBe("monitor:github-issue-audit:moda-labs%2Fbobi-agent%23519");
		});
		...
		it("gives distinct keys to separate findings from the same monitor", () => {
			...
			const second: NormalizedEvent = {
				...
				payload: {
					monitor: "github-issue-audit",
					finding_key: "moda-labs/bobi-agent#520",
				},
			};
