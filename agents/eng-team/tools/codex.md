...
`requires:` (installed + authed). There is no `connections:` block and no MCP
shim — you shell out to the CLI directly.
...
- If `codex` is missing or unauthed the preflight blocks dispatch — surface
  that, don't silently skip.
