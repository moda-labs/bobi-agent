...
| Release pipeline / packaging | — | #427, #543 | 🔴 #493 is closed; #427 remains design-first for library↔event-server protocol compatibility. #543 splits Codex-brain validation from SSH/local-team delivery validation and keeps bobi-agent release canary-specific. |
...
| Team distribution & composition | **Epic (#453)** | #440 → #446 → #451 → #452 | ✅ **DONE — CUTOVER COMPLETE & LIVE 2026-06-24.** PR #457 (`7709b76`) + **bobi 0.31.0 released** (compose on PyPI). moda-agents **PR #3 merged**, `BOBI_VERSION→0.31.0`, deploy dispatched (`only=eng-team`, `rebuild=true`) → live `moda-eng-team` reconciled onto the composed team. Behavioral identity confirmed live. |
...
`subscription-login-smoke + build-wheel + deploy-event-server → build-canary (smoke vs PROD event server) → publish → update-homebrew`.
...
| #543 | Separate Codex runtime validation from SSH/local-team delivery canary | 🟡 **OPEN — cleanup/design.** `codex-test` was a mixed signal (Codex brain + ssh-push + subscription + Slack). Keep bobi-agent release focused on `ci-canary`; split Codex-brain and SSH delivery into separate deterministic checks. |
...
(`from: eng-team` + Moda overlay, in private `moda-agents`) reconciled onto
...
| #452 | Std lib | Extract pristine `eng-team` (→ public bobi) + `moda-eng-team` overlay (→ private moda-agents) | ✅ **CLOSED + CUTOVER LIVE.** `agents/eng-team` extracted, monolith deleted, overlay PR merged, `deployments/eng-team.yaml` flipped to `team: moda-eng-team`, and live behavior verified. |
...
| #526 | Prep | Canonical `$BOBI_HOME/agents/<name>/{src,run/{package,state,workspace}}` layout; `BOBI_HOME` env-only override; `bobi agents` + `bobi agent <name>` CLI split; no fallback/backcompat path shims | 🟣 **IN REVIEW — PR #538.** Independent. Broad `paths.py`/CLI/runtime cutover. |
...
| #526 | Bobi Agent runtime layout + scoped CLI | PR #538 open; strict cutover, no cwd walk-up or backwards compatibility shims. |
