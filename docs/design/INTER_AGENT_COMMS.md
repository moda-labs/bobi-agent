...
3. **No durability, no observability.** The inbox queue is in-memory
   ...
   messages never appear in `bobi agent <name> events` or `events.jsonl` — only
...
4. **It's the odd one out.** Lifecycle `agent/*` events already publish through
   ...
   deployment WS → drain → inbox) and render in `bobi agent <name> events`. Direct
...
- **Synchronous ask** (`wait=True`): the handler holds the HTTP connection open
  ...
  `respond(msg_id, ...)` or it times out. Used by `bobi agent <name> message --wait`,
  `bobi agent <name> ask`, and `agents launch --wait`.
...
- The manager's well-known name (`manager`) makes `bobi agent <name> ask` /
...
**The hard case — the CLI is a one-shot process.** `bobi agent <name> ask "q" --wait`
...
- **T4 — observability + loop-safety coordination.** `inbox/*` in
  `bobi agent <name> events`; exempt from / aligned with #215 circuit breaker. *Gate:*
