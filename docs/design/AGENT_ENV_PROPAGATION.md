...
`bobi.env.child_agent_env()` is the single contract for parent-to-child
...
- `BOBI_ROOT` is always set to the spawner's bound installation root. A
  stale parent value is never inherited.
- `BOBI_BRAIN` is set from the installed team's `brain.kind` when present.
  This prevents a child for a Codex-backed team from inheriting a stale Claude
  selection, or the reverse.
- If the installed team has no `brain.kind`, `BOBI_BRAIN` is removed from
