...
- **Slack**: bot token referenced as `${SLACK_BOT_TOKEN}` in agent.yaml, value in `run/.env`. Handles workspace detection, webhooks, and replies.
- **Linear**: API key referenced as `${LINEAR_API_KEY}` in agent.yaml, value in `run/.env`. Handles team detection, webhooks, and writes.

Setup: paste the API key or token when `bobi agents install` prompts. Done.
...
## Installing a team: `bobi agents install`

Installing is distinct from authoring. A user with an existing team — from
the repo, a teammate, or a registry — runs:

```
$ bobi agents install agents/eng-team

Installed 'eng-team' into $BOBI_HOME/agents/eng/run/package/
...
Credentials saved to run/.env

Run `bobi agent <name> start` to launch.
```

Resolution order: local path first, then remote registries. Install:

1. Copies the team's `roles/`, `tools/`, `workflows/`, `monitors/`,
   `context/`, and `agent.md` into `run/package/` — the only runtime package
   location.
2. Writes the team's `agent.yaml` verbatim into `run/package/agent.yaml`
...
4. Scans the installed config for `${VAR}` references, prompts for any
   missing values, and writes them to `run/.env` (gitignored
   automatically).
5. Records a hash of every installed file in `install-manifest.json` —
   `bobi agent <name> doctor` flags hand-edits to the frozen image before a
   reinstall would silently destroy them.

`bobi agent <name> start` takes no arguments — it reads `run/package/agent.yaml`,
...
**Downloaded team** — installed from a registry. The cached source under
`$BOBI_HOME/cache/agents/` is the source of truth; reinstall regenerates
`run/package/`. Edit a source copy and reinstall rather than editing runtime
output in place. `run/.env` and `run/state/` are runtime-local.

**Local source of truth** — the team lives at `agents/<name>/` in the repo,
checked in. Install materializes it into `run/package/` as a build artifact:
...
Install writes `run/package/.gitignore` to match the path it took. Either
way, `run/package/agent.yaml` (the manifest — which team, its config,
...
terminal and writes `run/.env`; secret values never enter the
...
- **Customize** *(deferred — not yet implemented)* — loads the existing
  ...
  `run/package/` stays a frozen build artifact. Until it exists,
...
Installing into run/package/...
Done. Run `bobi agent <name> start` to launch.
```
...
normal local-source team, and `run/package/` stays a regenerable artifact.
...
re-running `bobi agents install` resets a hand-edited image back to its
source, and `bobi agent <name> doctor` flags drift against the install manifest.
...
On every `bobi agent <name> start`, the framework runs preflight checks before launching:
...
**Install copies** it verbatim into `run/package/agent.yaml`. The installed copy mimics a runtime installation — frozen, regenerated on every install, never edited in place. Customizing the team means editing the source `agents/<name>/agent.yaml` and reinstalling. Runtime package output is generated; source lives in `src/`, a checked-in team directory, or the registry cache.

**Secrets live in `run/.env`** — gitignored, created by `bobi agents install`, which scans the installed config for `${VAR}` references and prompts for each missing value. `Config.load()` reads `.env` into the environment before resolving the config, so every command (start, doctor, monitors) sees resolved values through a single path. `.env` is also where per-machine, non-secret variance goes — e.g. `event_server: ${BOBI_EVENT_SERVER}` resolves to the cloud Worker in production and to nothing (auto-started local server) in CI and local dev.
