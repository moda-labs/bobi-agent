...
ARG CODEX_VERSION=rust-v0.144.5
...
RUN arch="$(dpkg --print-architecture)" \
    ...
    && HOME=/tmp/smoke-home aichat --version \
    && rm -rf /tmp/smoke-home
...
RUN arch="$(dpkg --print-architecture)" \
    ...
    && HOME=/tmp/smoke-home codex --version \
    && rm -rf /tmp/smoke-home
...
# Build-time secrets reach the hook as BuildKit secrets (#35), never
# build-args: a build-arg persists in the image config / `docker history`,
# a secret mount exists only inside this RUN. The mounted value is exported
# into the hook's environment so a `run_root` step can reference e.g.
# ${MODA_SKILLS_TOKEN} (the private moda-skills clone). A hook that
# references a secret nobody supplied is refused host-side before docker
# runs (stage_team_deps in the deploy plugin); the hook's `set -u` is the
# backstop for builds driven outside `bobi build` / `bobi deploy`.
RUN --mount=type=secret,id=MODA_SKILLS_TOKEN \
    if [ -s /run/secrets/MODA_SKILLS_TOKEN ]; then \
        export MODA_SKILLS_TOKEN="$(cat /run/secrets/MODA_SKILLS_TOKEN)"; \
    fi \
    && bash /tmp/team-deps.sh && rm -f /tmp/team-deps.sh
...
# --- supervisor sidecar --------------------------------------------#
# The sidecar is the terminal process: it spawns + supervises the manager AND
# publishes tier-1 heartbeat + tier-2 lifecycle telemetry onto the bus.
#
# It needs no build step here. Since bobi 0.51.0 the sidecar ships INSIDE the
# wheel as `bobi.supervisor`, reached through the public CLI as
# `bobi agent <name> supervise`. The bundled-source COPY and the
# `/usr/local/bin/bobi-supervisor` PYTHONPATH shim that stood here existed only
# to carry a package that lived in a private repo; both are gone, along with the
# `BOBI_SUPERVISOR_SRC` ARG that let `bobi deploy` restage the tree. Whatever
# installs `bobi` above installs the supervisor with it, in every build mode.
...
# The entrypoint is PID 1 (under an injected init — Fly supplies one; a plain
# `docker run` needs `--init`, see docs/REFERENCE_IMAGE.md): it does root-only
# volume setup, then `exec gosu`s to the bobi user running the selected agent
# under `bobi agent <name> supervise`, so SIGTERM reaches the manager for
# graceful shutdown.
