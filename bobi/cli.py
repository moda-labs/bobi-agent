...
@click.option("--subscribe", multiple=True,
              help="Subscribe to event topics (e.g. moda-labs/bobi-agent-team, slack:T123)")
def agents_launch(workflow, role, run_key, task, timeout, wait, post_event, requested_by, non_interactive, persistent, subscribe):
    ...
    Examples:
        bobi agents launch -w issue-lifecycle --role engineer --id 42 --task "Fix moda-labs/bobi-agent-team#42"
        ...
        bobi agents launch -w adhoc --role manager --subscribe moda-labs/bobi-agent-team --persistent
    """
    if subscribe:
        persistent = True
