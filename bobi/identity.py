...
This lives at the top level rather than under ``bobi.supervisor`` because the
supervisor package eagerly imports ``.config`` and ``.supervision``, which a
one-shot leaf command (``bobi agent <name> otel ...``) must not pay for.
``bobi.supervisor.identity`` re-exports these names unchanged. Keep the imports
here to the standard library plus a lazy ``bobi.paths``: that is the whole
reason the module was promoted.
"""
