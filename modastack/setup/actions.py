...
`.modastack/.env`; only a masked echo is returned.
"""
...
def team_source_dir(project: Path, state: SetupState) -> Path:
    ...
    In **create** mode the chosen location is the *base* folder (e.g. `modastack/`);
...
def save_credential(state: SetupState, project: Path, var_name: str,
    ...
    if var.startswith("MODASTACK_"):
        raise ActionError("MODASTACK_* variables configure the framework "
...
def run_preflight(project: Path):
    """Run the same preflight checks `modastack start` runs. Returns the
