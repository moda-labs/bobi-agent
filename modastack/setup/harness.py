"""Harness status — is the AI agent that runs modastack reachable and authed?

The setup wizard's own digestion brain runs on the Claude Code CLI (see
``modastack/setup/llm.py``), and so does every agent in a team once it's
installed (``modastack/sdk.py``). So before the user gets anywhere, the
