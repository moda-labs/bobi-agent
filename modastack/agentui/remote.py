"""`modastack ui <deployment>` — tunnel to a deployed team's in-container UI.
...
`modastack deploy`), read the UI's port + token off the machine over `fly ssh`,
...
REMOTE_PROJECT = "/data/project"          # MODASTACK_PROJECT inside the container
DEFAULT_REMOTE_PORT = 8080


def _fly() -> str:
    from modastack.deploy import _fly_bin
    return _fly_bin()
...
def fetch_token(app: str, remote_project: str = REMOTE_PROJECT) -> str:
    return _ssh_cat(app, f"{remote_project}/.modastack/state/ui.token")


def fetch_remote_port(app: str, remote_project: str = REMOTE_PROJECT,
                      default: int = DEFAULT_REMOTE_PORT) -> int:
    raw = _ssh_cat(app, f"{remote_project}/.modastack/state/ui.port")
    try:
        return int(raw)
...
def _get_agents(local_port: int, token: str) -> dict:
    req = urllib.request.Request(
        ...
        headers={"x-modastack-ui-token": token})
...
def run(name: str | None = None, *, app: str | None = None,
    ...
    if not token:
        print(f"Couldn't read the UI token from '{target}'. The agent UI may not "
              "be enabled there — redeploy with MODASTACK_UI=1 (provisioned "
