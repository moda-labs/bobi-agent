"""The `modastack ui` web server — a minimal dashboard for a *running* team.
...
request/response**: a message is delivered via :func:`modastack.inbox.deliver`
...
- **local** (`modastack ui`): binds ``127.0.0.1:0`` in the foreground, mints a
  per-launch token, opens a browser. The operator's machine is the trust
  boundary, exactly like `modastack setup`.
- **container** (:func:`start_in_thread`, started by the manager when
  ``MODASTACK_UI`` is set): binds the Fly private 6PN address (``::``) in a
...
TOKEN_HEADER = "x-modastack-ui-token"
...
def _chat_log_path(project: Path, name: str) -> Path:
    return project / ".modastack" / "sessions" / name / "webui-chat.jsonl"
...
def serialize_card(entry, *, manager_name: str = "") -> dict:
    ...
    (`modastack.sdk.SessionEntry`) exposes; `is_manager` flags the entry-point
...
def serve(project: Path, *, mode: str = "local", open_browser: bool = True,
    ...
    """Run the UI in the foreground (the `modastack ui` command). Binds
    ...
    print(f"\n  modastack ui is running at {url}\n  (Ctrl-C to stop)\n")
...
def start_in_thread(project: Path, *, state_dir: Path,
    ...
    IMPORTANT: binds ``::`` (or ``$MODASTACK_UI_HOST``) — NOT ``127.0.0.1``,
    ...
    host = os.environ.get("MODASTACK_UI_HOST", "::")
    port = int(os.environ.get("MODASTACK_UI_PORT", "8080"))

    token = os.environ.get("MODASTACK_UI_TOKEN", "")
    if not token:
        token = secrets.token_urlsafe(24)
