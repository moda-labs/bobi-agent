"""State format version marker (.modastack/state/format_version).

Volumes outlive images — when the on-disk state format changes, a newer
modastack must know whether it can read the persisted state and an older
modastack must refuse to corrupt a format it doesn't understand.
...
- **Newer (on-disk > code):** refuse to start. The state was written
  by a newer modastack and may contain structures this version cannot
...
def ensure_state_version(root: Path | None = None) -> None:
    ...
    if fv.exists():
        ...
        if on_disk > CURRENT_FORMAT_VERSION:
            raise StateVersionError(
                f"State directory was written by a newer modastack "
                f"(format version {on_disk}, this build supports "
                f"{CURRENT_FORMAT_VERSION}). Upgrade modastack before "
