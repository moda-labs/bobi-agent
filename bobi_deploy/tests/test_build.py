"""Unit tests for the `bobi build` image engine (bobi_deploy/build.py, #610/#707).
...
bobi_deploy/tests/integration/test_bobi_build.py (docker-gated). Team
resolution (`bobi.build.resolve_team_dir`) stays public; its tests live in the
public suite and test_deploy_registry.py.
"""
...
def test_cli_build_wiring(tmp_path, monkeypatch):
    ...
    from bobi_deploy.cli import build as build_cmd
    ...
    monkeypatch.setattr("bobi_deploy.build.build_team_image", fake_build)
    result = CliRunner().invoke(build_cmd, [
        "./my-team", "--tag", "ghcr.io/a/t:1", "--push",
...
def test_cli_build_maps_errors(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("bobi_deploy.build.build_team_image", boom)
    result = CliRunner().invoke(build_cmd, ["./my-team"])
...
def test_resolve_assets_binary_mode_from_packaged(tmp_path, monkeypatch):
    ...
    # A fake packaged _deploy dir (what the bobi-deploy wheel ships).
    pkg = tmp_path / "_deploy"
